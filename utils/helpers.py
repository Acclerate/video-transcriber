...
def get_ffmpeg_version() -> Optional[str]:
    ...
    Returns:
        Optional[str]: FFmpeg 版本号，失败返回 None
    """
    try:
...
def get_ffmpeg_install_command() -> str:
    ...
    if system == "Linux":
        ...
        if machine.endswith("64"):
            return "1. 下载 FFmpeg: https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip\n2. 解压并添加到 PATH 环境变量"
        else:
            return "1. 下载 32位 FFmpeg: https://ffmpeg.org/download.html\n2. 解压并添加到 PATH 环境变量"
...
def print_dependency_check(console=None):
    """
    打印依赖检查结果

    Args:
        console: Rich Console 对象，如果为 None 则使用 print
    """
    all_ok, missing = check_dependencies()

    if all_ok:
        ffmpeg_version = get_ffmpeg_version()
        message = f"[dim]检测到 FFmpeg: {ffmpeg_version.split()[2] if ffmpeg_version else '未知版本'}[/dim]"
        if console:
            console.print(message)
        else:
            print(f"✓ FFmpeg 可用 ({ffmpeg_version.split()[2] if ffmpeg_version else '未知版本'})")
    else:
        if console:
